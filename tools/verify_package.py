...
documented contract. Same author as the producer, so this is N-version, not independent.
  ...
  python tools/verify_package.py PACKAGE.json      exit 0 all checks pass | 1 a check failed | 2 unreadable
...
SCHEMA = "sv.package/0"
REQUIRED_LIMITS = ("freshness:", "authenticity:", "verifier identity:", "resource state:")
...
def verify(pkg):
    ...
    if out is None:
        check("measurement_recomputed", True, f"kind {m.get('kind')!r} not recomputable - recorded only")
    else:
    ...
    check("limitations_declared", all(any(l.startswith(p) for l in lims) for p in REQUIRED_LIMITS))
    return checks
